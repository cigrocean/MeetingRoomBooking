...
import { fetchRooms, fetchBookings, getRoomStatus, createBooking, fetchAvailableTimeSlots, getSheetUrl } from '../services/googleSheets';
    ...
    <div className="container py-8">
        ...
        <div className="flex items-center gap-2 sm:gap-4 w-full justify-between flex-wrap">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-2 sm:px-3 py-1 sm:py-1.5 rounded-md bg-surface-hover hover:bg-primary/20 transition-all border border-slate-700 hover:border-primary text-xs sm:text-sm text-white whitespace-nowrap"
              title={t('language')}
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          ...
          rooms.map(room => {
            const { status, nextBooking } = getRoomStatus(room.id, bookings);
